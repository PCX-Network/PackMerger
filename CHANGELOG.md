...
- **Bedrock / Geyser conversion (opt-in, experimental).** New `:bedrock` module
...
- **Velocity proxy module (network-wide distribution).** New `PackMerger-Velocity`
  ...
  the `packmerger:pack` plugin channel. Shares a new `:common` module (the
...
- **Monorepo.** Restructured into a Gradle multi-module build; the plugin now lives
  in the `:plugin` module (jar still ships as `PackMerger-<version>.jar`, now under
  `plugin/build/libs/`). Reserves room for the planned `:bedrock` and `:proxy-*`
  modules. Dependency versions remain in the shared root `gradle/libs.versions.toml`.
