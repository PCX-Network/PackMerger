...
### Plugin Pack Auto-Discovery

If you run other plugins that generate their own resource pack — Oraxen, Nexo,
ItemsAdder, ModelEngine, EliteMobs, FreeMinecraftModels — PackMerger finds each
one's current pack and stages it into `packs/.plugin-packs/<alias>.zip` before
every merge, so you never have to export and copy files by hand. Staged packs go
through the **same deep-merge** as everything else, so two plugins that both touch
`assets/minecraft/items/*` or share model overrides combine cleanly instead of one
silently winning.

Reference a plugin by its alias (`oraxen`, `nexo`, `itemsadder`, `modelengine`,
`elitemobs`, `freeminecraftmodels`) anywhere in `priority:` to set its precedence.
Per-plugin overrides live under `plugin-packs.sources.<alias>` — set `enabled: false`
to skip one, or `path:` to point at a non-default output location. Because other
plugins build their packs as they start up, the first merge is delayed by
`plugin-packs.merge-delay-seconds`; after rebuilding a plugin's pack (e.g.
`/oraxen reload`), run `/pm` reload/merge to pull in the fresh output.
